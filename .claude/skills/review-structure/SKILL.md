...
description: 审查 BDGO 代码库的结构性问题 — 分层、耦合、模块边界、命名一致性、循环依赖。当用户说"审查结构"、"看看架构"、"检查分层"、"review structure"、"structure review"时触发。不做性能/可维护性/API/安全审查（分别用对应 /review-* 或 /security-review skill）。
---
...
#### E. 结构层面的重复（死代码类见 /review-maintainability A 类）
- **E1** 两个 router 是否实现几乎相同的逻辑（复制粘贴信号，结构层面缺少共享抽象）
- **E2** 同一 SQL 查询 / 同一业务规则在 ≥ 2 处重复 → 列出并指出应提取到哪个 service

#### F. 测试结构
- **F1** `api/tests/` 是否镜像 `api/` 目录结构
- **F2** service 层是否有独立单元测试，还是只有通过 router 的集成测试（伪测试检查见 /review-maintainability E1）
...
新 finding 追加到 `docs/review-findings.md`。该文件是 Schema / 状态词汇 / Severity 口径的**单一真源** —— 本 skill 不在此重复定义，以 findings.md 为准。ID 前缀用 `S-xxx`。如果文件不存在，**提示用户**是否创建，不要擅自建目录。

## 严重度判定（遵循 findings.md Severity 口径；下面补 structure 专用具体判据）

- **critical**: 关键路径（auth / credits / rate_limit）上的循环依赖或分层彻底破坏

...
function CapabilityMap({ data }: { data: Record<string, string> }) {
  if (!data || typeof data !== "object") return <span style={{ color: "var(--text-secondary)" }}>-</span>;
  const colorMap: Record<string, string> = {
    strong: "#10b981", moderate: "#f59e0b", minimal: "#94a3b8", growing: "#3b82f6",
  };
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
      {Object.entries(data).map(([ta, level]) => (
        <span
          key={ta}
          style={{
            padding: "0.2rem 0.6rem", borderRadius: 9999, fontSize: "0.75rem", fontWeight: 500,
            background: `${colorMap[level] || "#94a3b8"}20`, color: colorMap[level] || "#94a3b8",
...
function BDTheses({ data }: { data: any[] }) {
  if (!Array.isArray(data) || data.length === 0) return <span style={{ color: "var(--text-secondary)" }}>-</span>;
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
      {data.map((thesis, i) => (
        <div key={i} style={{ padding: "0.6rem", background: "var(--bg)", borderRadius: 8, fontSize: "0.85rem" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
      {Object.entries(data).map(([ta, info]) => (
        <div key={ta} style={{ display: "flex", justifyContent: "space-between", padding: "0.4rem 0.6rem", background: "var(--bg)", borderRadius: 6, fontSize: "0.85rem" }}>
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
      {Object.entries(data).sort((a, b) => b[1] - a[1]).map(([type, count]) => (
        <span key={type} style={{ padding: "0.2rem 0.6rem", background: "var(--bg)", borderRadius: 6, fontSize: "0.8rem" }}>
...
export default function BuyerDetailPage() {
  ...
  useEffect(() => {
    fetchBuyer(name).then(setBuyer).catch(() => setNotFound(true));
  }, [name]);

  const capabilities = useMemo(() => buyer ? safeJsonParse(buyer.commercial_capabilities) : null, [buyer]);
  ...
  const dealTypePref = useMemo(() => buyer ? safeJsonParse(buyer.deal_type_preference) : null, [buyer]);
  ...
  return (
    <div>
      <div className="detail-header">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <button
            ...
            style={{ padding: "0.4rem 0.9rem", background: "var(--accent)", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontSize: "0.8rem", fontWeight: 600 }}
          >
            View Company
          </button>
        </div>
      ...
      {sunkCost && Object.keys(sunkCost).length > 0 && (
        <div className="card" style={{ marginBottom: "1rem" }}>
          <h3 style={{ margin: "0 0 0.75rem", fontSize: "0.95rem" }}>Investment by Therapeutic Area</h3>
      ...
      <div className="card">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "0.6rem", fontSize: "0.85rem" }}>

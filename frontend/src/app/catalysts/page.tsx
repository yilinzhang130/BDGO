...
function PhaseBadge({ phase }: { phase: string }) {
  ...
  const colors: Record<string, string> = {
    "Phase 3": "#059669", "Phase 2": "#2563eb", "Phase 1": "#7c3aed",
    "Pre-clinical": "#6b7280", "Commercial": "#0d9488",
  };
  ...
  return (
    <span style={{
      ...
      color, border: `1px solid ${color}33`, backgroundColor: `${color}0d`,
    }}>
      {phase}
    </span>
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        gap: 12, padding: "12px 16px", borderBottom: "1px solid #f3f4f6",
    ...
    >
      ...
      <div style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.3 }}>
        {(event.indication || "").slice(0, 50)}{(event.indication || "").length > 50 ? "…" : ""}
      </div>
      {/* Phase */}
      <div><PhaseBadge phase={event.phase} /></div>
      {/* Status */}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "grid", gap: 16 }}>
        ...
        <Field label="Company" value={event.company} link={`/companies/${encodeURIComponent(event.company)}`} />
...
function Field({ label, value, link }: { label: string; value?: string; link?: string }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 11, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", marginBottom: 2 }}>
        {label}
      </div>
      {link ? (
        <a href={link} style={{ fontSize: 14, color: "#2563eb", textDecoration: "none" }}>{value}</a>
      ) : (
...
function Backdrop({ onClick }: { onClick: () => void }) {
  return (
    <div onClick={onClick} style={{
      ...
      backgroundColor: "rgba(0,0,0,0.2)", zIndex: 49,
    }} />
  );
}
...
function MonthGrid({ events, year, month }: { events: any[]; year: number; month: number }) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    cells.push(
      <div key={d} title={info ? `${info.count} catalyst(s)` : undefined} style={{
        width: 28, height: 28, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 11, fontWeight: info ? 600 : 400,
    ...
    <div style={{ minWidth: 220 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 28px)", gap: 2 }}>
        {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
          <div key={i} style={{ fontSize: 10, color: "#9ca3af", textAlign: "center", fontWeight: 600 }}>{d}</div>
        ))}
        {cells}
      </div>
    </div>
...
export default function CatalystsPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetchCatalysts({
      q, status_filter: statusFilter, phase, catalyst_type: catalystType,
      year, page, page_size: 200,
  ...
  return (
    <div className="page-container" style={{ maxWidth: 1100 }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        <StatCard label="Total Catalysts" value={stats.total} color="#111"
          active={statusFilter === ""} onClick={() => { setStatusFilter(""); setPage(1); }} />
        <StatCard label="Overdue" value={stats.overdue} color="#dc2626"
          active={statusFilter === "overdue"} onClick={() => { setStatusFilter(statusFilter === "overdue" ? "" : "overdue"); setPage(1); }} />
        <StatCard label="Imminent (< 30d)" value={stats.imminent} color="#d97706"
          active={statusFilter === "imminent"} onClick={() => { setStatusFilter(statusFilter === "imminent" ? "" : "imminent"); setPage(1); }} />
        <StatCard label="Upcoming (30–90d)" value={stats.upcoming} color="#2563eb"
          active={statusFilter === "upcoming"} onClick={() => { setStatusFilter(statusFilter === "upcoming" ? "" : "upcoming"); setPage(1); }} />
        <StatCard label="Far (> 90d)" value={stats.far} color="#6b7280"
          active={statusFilter === "far"} onClick={() => { setStatusFilter(statusFilter === "far" ? "" : "far"); setPage(1); }} />
      </div>

      {/* Filters */}
      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
        <input
          type="text" placeholder="Search company, asset, event..."
        ...
        <select value={catalystType} onChange={e => { setCatalystType(e.target.value); setPage(1); }}
          style={{ padding: "8px 12px", borderRadius: 8, border: "1px solid #e5e7eb", fontSize: 13 }}>
      ...
      {/* Content */}
      {loading ? (
        <div style={{ textAlign: "center", padding: 60, color: "#9ca3af" }}>Loading catalysts...</div>
      ) : view === "list" ? (
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb", overflow: "hidden" }}>
          {/* Table header */}
          <div style={{
            display: "grid", gridTemplateColumns: "110px 1fr 140px 100px 90px",
            gap: 12, padding: "10px 16px", background: "#f9fafb", borderBottom: "1px solid #e5e7eb",
          ...
          {events.length === 0 ? (
            <div style={{ padding: 40, textAlign: "center", color: "#9ca3af" }}>No catalysts found</div>
          ) : (
            events.map((e: any) => (
          ...
          {data && data.total > 200 && (
            <div style={{ display: "flex", justifyContent: "center", gap: 8, padding: 16 }}>
              ...
              <button disabled={page * 200 >= data.total} onClick={() => setPage(page + 1)}
                style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid #e5e7eb", cursor: "pointer" }}>
                Next →
              </button>
            </div>
        ...
        <div style={{
          ...
          gap: 20, padding: 16, background: "#fff", borderRadius: 12, border: "1px solid #e5e7eb",
        }}>

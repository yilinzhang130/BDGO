...
export const DOCS_TOC: TocItem[] = [
  ...
  { id: "outreach-workspace", title: "Outreach 工作台（即将上线）", level: 2 },
];
...
| **外联与执行** | \`/email\` \`/batch-email\` \`/outreach\` \`/log\` \`/import-reply\` **[迁出中]** \`/timing\` |
...
| \`/meeting\` | 会前简报 |
| \`/email\` **[迁出中]** | 外联邮件初稿 → 即将由 Outreach 工作台替代 |
| \`/batch-email\` **[迁出中]** | 批量外联邮件 → 即将由 Outreach 工作台替代 |
| \`/outreach\` **[迁出中]** | 外联管线追踪 → 即将由 Outreach 工作台替代 |
| \`/log\` **[迁出中]** | 外联记录日志 → 即将由 Outreach 工作台替代 |
| \`/import-reply\` **[迁出中]** | 邮件回复自动归档 → 即将由 Outreach 工作台替代 |
| \`/timing\` | 外联时机建议 |
...
## <a id="outreach-workspace"></a>Outreach 工作台（即将上线）

Outreach 工作台将把现有 5 个外联 slash 命令整合为一个独立的可视化工作区，告别命令行操作，所有外联全流程在同一界面完成。

### 规划功能

| 模块 | 说明 |
|---|---|
| **Pipeline 视图** | 看板式外联漏斗——Identified / Contacted / Replied / Meeting / Closed，拖拽更新状态 |
| **Compose 表单** | 内置 AI 写信助手，选模板（Cold / Follow-up / Re-engagement）→ 一键生成 + 手动调整 → 直接发送或导出 |
| **粘贴回信** | 将对方邮件内容粘贴进来，AI 自动识别意向、更新 pipeline 状态、建议下一步行动 |
| **批量外联** | CSV 上传买方名单 → 变量替换 → 预览 → 定时发送，全程去重与频控 |

> **当前过渡安排**：上线前，\`/email\`、\`/log\`、\`/outreach\`、\`/import-reply\`、\`/batch-email\` 继续可用，但标记为 **[迁出中]**。Outreach 工作台上线后将隐藏这 5 个命令。迁移期预计 2 周，届时会有 banner 提示。

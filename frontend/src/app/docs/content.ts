...
## <a id="reports"></a>11 个报告命令

在对话输入框里键入 \`/\` 唤出命令面板。这些是我们把最高频 BD 场景固化成的"一键模板"，每个都调用专门的 Agent 链路。

| 命令 | 产物 | 典型用途 |
|---|---|---|
| \`/mnc\` | MNC 买方画像 | 管线策略 + 历史 BD 图谱 + 缺口分析 |
| \`/dd\` | Due Diligence 清单 | 标的尽调关键点梳理 |
| \`/evaluate\` | 资产交易吸引力评估 | 科学、临床、商业、交易四象限 |
| \`/rnpv\` | rNPV 估值模型 | 风险调整 NPV，输出 Excel |
| \`/teaser\` | 资产 Teaser | 一页纸对外推介材料 |
| \`/target\` | 靶点雷达 | 靶点全球竞争格局 |
| \`/disease\` | 疾病领域概览 | 适应症全景 + 关键玩家 |
| \`/commercial\` | 商业化机会评估 | 市场规模 + 竞争 + 定价 |
| \`/paper\` | 文献分析 | 深度拆解一篇论文及其影响 |
| \`/ip\` | IP 专利格局 | 核心专利族 + 到期时间 + FTO 提示 |
| \`/guidelines\` | 临床指南摘要 | 最新治疗指南要点 |
...
### <a id="other-reports"></a>其他报告命令

完整参数与示例请在对话里直接使用 \`/\` 命令查看即时提示。所有报告都是 **可追溯、可引用、可导出**——每个结论都带来源链接。

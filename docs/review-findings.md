...
| SEC-002 | 2026-04-25 | security | A2/D3 | high | done | S | n/a | api/routers/auth.py:88-142 | POST /login 和 /register 无速率限制；允许暴力破解密码及 bcrypt CPU 耗尽型 DoS | PR #79: 模块级 `_auth_rate_limit` per-IP 滑动窗口计数器（10次/60s），超限返回 429 + Retry-After；注释说明多 worker 局限性 |
| SEC-003 | 2026-04-25 | security | A2 | medium | done | S | n/a | api/routers/auth.py:100 + api/auth.py:36 | register/login 在调用 bcrypt 前无密码长度上限；虽 bcrypt 截断 72 字节，但超长字符串的 encode() 开销在并发时仍构成放大攻击面 | PR #79: `RegisterRequest.password` 和 `LoginRequest.password` 加 `Field(max_length=128)`，Pydantic 在进入 bcrypt 前拒绝超长密码 |
| SEC-004 | 2026-04-25 | security | C1/D1 | medium | done | M | n/a | api/credits.py:93-104 + 123-178 | TOCTOU：`ensure_balance` 在事务1读余额，LLM 运行期间不持锁，`record_usage` 在事务2扣款；并发请求均通过检查后同时扣款，导致超额消费；`GREATEST(balance-%s,0)` 防负但不防超用 | PR #79: `record_usage` 内加 `SELECT balance FOR UPDATE` 序列化扣款；检测到 balance_before < credits_charged 时写 `CREDIT_RACE_DETECTED` 警告 |
| SEC-005 | 2026-04-25 | security | A1/B2 | medium | done | S | n/a | api/routers/upload.py:24-67 | `upload_bp` 无 `Depends(get_current_user)` → 无法记录上传者身份；`serve_bp` / `list_bp_files` 向所有已认证用户开放全部 BP 文件，无用户维度隔离 | PR #80: upload_bp 注入 `user: dict = Depends(get_current_user)`，emit `bp_upload` 结构化日志；serve/list 文档说明共享意图 |
| SEC-006 | 2026-04-25 | security | A2/D3 | medium | done | S | n/a | api/routers/upload.py:43-46 | `upload_bp` 流式写入磁盘无总大小限制；攻击者可上传数 GB 文件耗尽磁盘空间 | PR #80: 累计 size 超过 `_MAX_UPLOAD_BYTES`（50 MB）时 raise 413，delete 已写入的临时文件后再抛出 |
| P-014 | 2026-04-25 | performance | B1 | medium | done | S | n/a | api/routers/sessions.py:203-208 | `GET /api/sessions/{session_id}` 无 LIMIT 拉取全部消息；长期活跃 session 可积累数千条消息，单次查询返回全量数据，内存和传输无界 | PR #80: `ORDER BY created_at DESC LIMIT 200`，Python `reversed()` 恢复升序返回；客户端已有前序消息缓存 |
| A-002 | 2026-04-25 | api-design | A3/C1 | low | done | S | n/a | api/routers/watchlist.py:47 | `GET /watchlist?type=invalid_type` 静默忽略无效 type 并返回全量数据，而非 400；与 POST /watchlist 对 entity_type 的严格校验不一致 | PR #81: `type` 有值但不在 VALID_ENTITY_TYPES 时 raise HTTPException(400)，提示合法值列表 |
| M-026 | 2026-04-25 | maintainability | G2 | medium | done | S | n/a | api/main.py:117 + api/services/report_builder.py:430 | `reclaim_stale_tasks` 仅在启动时调用一次；启动后挂起的任务直到下次重启前永远停留 running 状态，长期运行的部署会积累僵尸报告行 | PR #81: lifespan 中 `asyncio.create_task(_periodic_reclaim())` 每 15 分钟执行一次；shutdown 时 cancel + await |
| M-027 | 2026-04-25 | maintainability | A4 | low | done | S | n/a | api/routers/watchlist.py:19-21 | `WatchlistAdd` 的 `entity_key` 和 `notes` 无 `max_length` 约束；LLM 工具调用可能传入任意长字符串导致 DB 列溢出或内存激增 | PR #81: `entity_key = Field(..., max_length=500)`，`notes = Field(None, max_length=2000)` |
| M-028 | 2026-04-25 | maintainability | E2 | low | done | S | n/a | frontend/src/hooks/usePaginatedTable.ts:57 | `load` useCallback 的 deps 数组加了 `eslint-disable-next-line` 注释屏蔽警告；若调用方传入不稳定的 `fetchFn`（每次渲染新建函数引用），hook 会在每次渲染时重新创建 load 并无限触发请求 | PR #81: 去掉 eslint-disable 注释（deps 数组本就正确）；JSDoc 新增 ⚠️ STABILITY REQUIREMENT 章节要求 `fetchFn` 必须稳定 |

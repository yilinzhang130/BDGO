...
| M-008 | 2026-04-24 | maintainability | B1/B2/B4 | medium | in-progress | L | n/a | api/services/document/rnpv_excel.py | Started at 24 C901 > 10. PRs #35, #48, #50, #51 (rnpv_sheet 24→<10). PR for build_pl (was 17 → now `rnpv/pl.py` with `_write_currency_row` helper powering every line — same shape Revenue/COGS/GP/R&D/SG&A/EBIT/Tax/FCF). 5 C901 sheet builders still in rnpv_excel.py: `build_qc=15, build_patient_funnel=13, build_revenue=13, build_sensitivity=12, build_references=12` | Continue one per PR. `build_qc_sheet` next (reads mirror + tracker — no new downstream deps) |

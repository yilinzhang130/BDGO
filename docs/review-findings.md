...
| M-004 | 2026-04-24 | maintainability | D5/D6 | high | open | L | n/a | frontend/src (120 eslint hits) | Frontend has 120 `@typescript-eslint/no-explicit-any` warnings + 111 literal `any` occurrences. Heaviest users: `app/catalysts/page.tsx` (event/events typed as any[]), `app/admin/page.tsx` (6 `catch (e: any)`), `app/dashboard/page.tsx` (recharts render props). Types are effectively advisory — API response shapes get no compile-time check | Start with `lib/api.ts` (API response models) since every page inherits its `any` leak. Then sweep `catch (e: any)` to `catch (e: unknown)` + narrow. Don't chase the whole 120 in one PR |

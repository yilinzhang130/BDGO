...
| M-008 | 2026-04-24 | maintainability | B1/B2/B4 | medium | in-progress | L | n/a | api/services/document/rnpv_excel.py | Started at 24 C901 > 10. PR #35 split `call_minimax_stream`. PR #48 split `build_assumptions_sheet` (was 32); PR #50 split `build_cost_sheet` (was 26) — both into `services/document/rnpv/*.py` with shared `_styles.py` + `_helpers.py`. 7 C901 sheet builders still in rnpv_excel.py: `build_rnpv=24, build_pl=17, build_qc=15, build_patient_funnel=13, build_revenue=13, build_sensitivity=12, build_references=12` | Continue one `build_*_sheet` per PR. `build_rnpv_sheet` next (reads cost_sheet's CellTracker refs — now sourced from `rnpv/cost.py`) |

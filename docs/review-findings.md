...
| M-008 | 2026-04-24 | maintainability | B1/B2/B4 | medium | in-progress | L | n/a | api/services/document/rnpv_excel.py | Started at 24 C901 > 10. 9/10 rNPV sheet builders now extracted into `api/services/document/rnpv/*.py` (assumptions/patient_funnel/revenue/cost/pl/rnpv_sheet/sensitivity/qc/references) via PRs #35, #48, #50, #51, #55, #56, #57, #59, #60, and build_revenue (was 13). rnpv_excel.py dropped 2491 → 282 lines; retains imports, CellTracker, small `build_summary_sheet` (already C<10), `generate()`, CLI `main()`. **Zero C901 > 10 in rnpv_excel.py.** Only remaining C901 in services/document/ is the unrelated `markdown_to_docx=19` in docx_builder | Close M-008 once the revenue PR merges. `markdown_to_docx` belongs in a separate finding — docx flow, not rNPV |

...
| M-008 | 2026-04-24 | maintainability | B1/B2/B4 | medium | in-progress | L | n/a | api/services/document/rnpv_excel.py | Started at 24 C901 > 10. PRs #35 (call_minimax_stream), #48 (assumptions 32→<10), #50 (cost 26→<10), and build_rnpv (was 24) extracted to `services/document/rnpv/rnpv_sheet.py` with Excel-write helpers + Python-mirror FCF/NPV split. 6 C901 sheet builders still in rnpv_excel.py: `build_pl=17, build_qc=15, build_patient_funnel=13, build_revenue=13, build_sensitivity=12, build_references=12` | Continue one `build_*_sheet` per PR. `build_pl_sheet` next — it reads revenue/cost trackers and writes `pl.fcf.y{n}` consumed by the rNPV mirror |

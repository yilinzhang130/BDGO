...
| M-008 | 2026-04-24 | maintainability | B1/B2/B4 | medium | in-progress | L | n/a | api/services/document/rnpv_excel.py | Started at 24 C901 > 10. PRs #35, #48, #50, #51, #55 (pl 17→<10). build_qc (was 15) now in `rnpv/qc.py` — each QC check is its own pure fn returning a `(name, status, detail, expected)` tuple; `_collect_checks` assembles, `_write_check_row` dispatches by PASS/FAIL/WARN. 4 C901 sheet builders still in rnpv_excel.py: `build_patient_funnel=13, build_revenue=13, build_sensitivity=12, build_references=12` | Continue one per PR. `build_references_sheet` next (smallest remaining at C=12, pure docs writer) |

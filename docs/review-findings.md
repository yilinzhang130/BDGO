...
| M-008 | 2026-04-24 | maintainability | B1/B2/B4 | medium | in-progress | L | n/a | api/services/document/rnpv_excel.py, api/routers/chat/llm_stream.py | Started at 24 C901 functions > 10. `call_minimax_stream` split landed in PR #35. `build_assumptions_sheet` (was 32, PR #48) and `build_cost_sheet` (was 26) extracted to `rnpv/assumptions.py` + `rnpv/cost.py` with shared `rnpv/_styles.py` + `rnpv/_helpers.py`; all sub-helpers C<10. 7 C901 warnings remain in rnpv_excel.py (build_rnpv=24, build_pl=17, build_qc=15, build_patient_funnel=13, build_revenue=13, build_sensitivity=12, build_references=12). One sheet per follow-up PR to keep churn reviewable | Continue one `build_*_sheet` per PR into `api/services/document/rnpv/`. `build_rnpv_sheet` next (depends on cost_sheet's cell refs via CellTracker, now in rnpv/cost.py) |
...
| M-020 | 2026-04-24 | maintainability | G4 | low | in-progress | S | n/a | api/services/document/rnpv_excel.py:2452-2468 | `generate()` prints 8+ status lines (`✓ rNPV model saved`, rNPV/peak rev/QC/references summary) directly to stdout. It's called by `services/reports/rnpv_valuation.py:279` inside the FastAPI report pipeline, so these unstructured prints leak into the API service log alongside the JSON-formatted records — unfilterable, no level, no request_id | Swap `print(...)` → `logger.info(...)` (create a module logger). The CLI `main()` block at line 2470 can still call the same function; prints are not worth preserving for interactive use given this is also a library entry point |

...
| M-008 | 2026-04-24 | maintainability | B1/B2/B4 | medium | in-progress | L | n/a | api/services/document/rnpv_excel.py, api/routers/chat/llm_stream.py | Started at 24 C901 functions > 10. `call_minimax_stream` split landed in PR #35. `build_assumptions_sheet` (was 32) extracted to `rnpv/assumptions.py` with shared `rnpv/_styles.py` + `rnpv/_helpers.py`; all sub-helpers C<10. 8 C901 warnings remain in rnpv_excel.py (build_cost=26, build_rnpv=24, build_pl=17, build_qc=15, build_patient_funnel=13, build_revenue=13, build_sensitivity=12, build_references=12). One sheet per follow-up PR to keep churn reviewable | Continue one `build_*_sheet` per PR into `api/services/document/rnpv/`. `build_cost_sheet` next (rnpv_sheet depends on its cell refs via CellTracker, so cost before rnpv) |

...
| M-008 | 2026-04-24 | maintainability | B1/B2/B4 | medium | in-progress | L | n/a | api/services/document/rnpv_excel.py | Started at 24 C901 > 10. PRs #35, #48, #50, #51, #55, #56, #57 (references 12→<10). build_sensitivity (was 12) now in `rnpv/sensitivity.py` — `_make_quick_rnpv` closure factored out of the inner-fn tangle; tornado vars now a data table with per-var format string; chart/scenarios each their own writer. 2 C901 sheet builders still in rnpv_excel.py: `build_patient_funnel=13, build_revenue=13` | Continue one per PR. `build_patient_funnel_sheet` next |

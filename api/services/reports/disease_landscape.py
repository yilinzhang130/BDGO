...
7. **数据缺失直接说** — 标注 "[BDGO数据稀疏]" 或 "[未获取相关信息]"，不要编造
"""
...
_GAP_FILL_PROMPT = """以下是已生成的疾病领域全景报告 markdown 草稿，以及 Schema 校验器发现的结构性缺陷列表。
...
修补规则：
- "section_missing" → 按 9 章顺序（文献综述 / 已批准药物 / Phase3管线 / Phase1/2管线 /
  关键催化剂 / 治疗指南 / 未满足需求 / 新兴靶点 / 立项评分矩阵）插入缺失章节
- 已批准药物章必须含表格（药物名 / 公司 / 靶点 / 获批年份 / 关键数据）
- 治疗指南章必须引用主流指南（NCCN/ASCO/ESMO/AHA 等之一）
- 立项评分章必须含数字化评分矩阵（总分 / 各维度得分）
- 数据驱动 — 若数据缺失明确写 "⚠️ 待补充"，不要编造
- 输出整个 markdown，不加任何解释或代码块包裹
"""


def _build_disease_gap_fill_prompt(markdown: str, audit) -> str:
    fail_lines = [
...
class DiseaseLandscapeService(ReportService):
    ...
    def run(self, params: dict, ctx: ReportContext) -> ReportResult:
        ...
        if not enum and not keyword:
            raise ValueError(
                "Could not resolve disease input. Try 'Oncology', 'NSCLC', 'NASH', etc."
            )
        ...
        header = (
            ...
            f"**数据源**: BDGO数据库 ({len(assets)} 个资产, {len(trials)} 条临床, "
    ...
    def _validate_and_repair(
        ...
        try:
            audit = validate_markdown(markdown, mode="disease_landscape")
            ...
            patched = ctx.llm(
                system=system_prompt,
                messages=[
                    {"role": "user", "content": _build_disease_gap_fill_prompt(markdown, audit)}
                ],
                max_tokens=8000,
                label="disease_gap_fill",
            )
            if len(patched) > 500:
                audit2 = validate_markdown(patched, mode="disease_landscape")
                ctx.log(
    ...
    def _query_assets(self, ctx: ReportContext, enum: str, keyword: str, limit: int) -> list[dict]:
        ...
        if keyword:
            conds.append(
                f'("适应症" LIKE ? {LIKE_ESCAPE} OR "靶点" LIKE ? {LIKE_ESCAPE} OR "作用机制(MOA)" LIKE ? {LIKE_ESCAPE})'
            )
    ...
    def _query_companies(self, ctx: ReportContext, enum: str, keyword: str) -> list[dict]:
        ...
        if keyword:
            conds.append(
                f'("疾病领域" LIKE ? {LIKE_ESCAPE} OR "核心产品的阶段" LIKE ? {LIKE_ESCAPE} OR "主要核心pipeline的名字" LIKE ? {LIKE_ESCAPE})'
            )
            params.extend([like_contains(keyword)] * 3)
        if not conds:
            return []

"""
chat_store.py — Internal persistence layer for the chat streaming engine.

Owns every SQL statement that the chat feature writes to Postgres:
  - ensure_session / load_history / save_message
  - save_entities (context-panel state)
  - get_session_brief / save_session_brief (compaction cache)

INTERNAL USE ONLY — called exclusively by the chat streaming engine
(streaming.py, compaction.py, entities.py, quick_search.py).
Exceptions are logged, never re-raised — streaming must degrade
gracefully if the DB hiccups mid-response.

Not the same as ``routers/sessions.py``:
  - chat_store   = storage primitives, no HTTP, no auth checks
  - sessions.py  = REST API (CRUD endpoints the frontend calls directly)
"""
